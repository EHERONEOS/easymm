<template>
  <div style="background-color:#E5E5E5">
      ...
      <input type="hidden" name="contact_city_code" id="contact_city_code" value="" data-city-name="">
			<input  data-city-code="c0" data-province-code="p0" data-district-code="t0"  id="addaddress_dz" placeholder="请输入地址" v-model="address"  readonly="readonly">
		</div>
  ...
  mounted() {
      ...
      success: data => {
        if (data.isSuccess) {
          for (let i = 0; i < data.result.length; i++) {
            self.iosProvinces.push({
              id: "p" + i,
              value: data.result[i].name,
              parentId: 0
            });
            for (let j = 0; j < data.result[i].areas.length; j++) {
              self.iosCitys.push({
                id: "c" + j,
                value: data.result[i].areas[j].name,
                parentId: "p" + i
              });
              for (let k = 0; k < data.result[i].areas[j].areas.length; k++) {
                self.iosCountys.push({
                  id: "t" + k,
                  value: data.result[i].areas[j].areas[k].name,
                  parentId: "c" + j
                });
              }
            }
    ...
    selectContactDom.bind("click", function() {
      ...
      var iosSelect = new IosSelect(
        3,
        [self.iosProvinces, self.iosCitys, self.iosCountys],
        {
          title: "地址选择",
          itemHeight: 35,
          relation: [1, 1],
