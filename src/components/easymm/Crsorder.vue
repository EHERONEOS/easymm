		...
		<div id="main">
			<div id="yhxx" >
				...
				<div class="grtime">
					...
					<div class="Time">
						<div class="tt">
							<input type="text"  id="left" class="rq" name="appDate" v-model="starttime">
						</div>
						<div class="zhsj">
							<div>开始时间</div>
						</div>
...
export default {
  ...
  methods: {
    appointment() {
      ...
      $.ajax({
        ...
        success: data => {
          if (data.isSuccess) {
            var redirect_uri =
              "http://www.qsmam.com/mobile/weixinH5/page/paymentOrder.html?data={orderId:" +
              data.result.orderId +
              ",pre:" +
              self.dj +
              ",job:" +
            ...
            // location.href="orddetal.html?id="+data.result+"&appToken="+appToken
          } else {
    ...
    price() {
      ...
      console.log(self.nurseoneprice);
      self.hjprice = Math.round(price + parseInt(self.nurseoneprice));
      self.dj = self.hjprice;
    },
    Prompt(text) {
      this.$dialog.toast({
        mes: text,
...
<style scoped>
@import url(../../../static/css/easymm/Crsorder.css);
</style>